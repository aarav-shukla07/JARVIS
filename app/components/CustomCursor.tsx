...
export default function CustomCursor() {
  ...
  const leftColors = ["#492701", "#f7a241", "#42c183"]; 
  ...
  return (
    ...
    >
      {/* HELLO tag */}
      <div
        className={`absolute transition-opacity transition-transform duration-300 ease-in-out px-2 py-[2px] border border-white rounded-md text-xs font-mono uppercase bg-black/60 text-white ${
          showHello ? "opacity-100 scale-100" : "opacity-0 scale-90"
        }`}
      ...
      {/* Cursor circle */}
      <div
        className={`absolute w-3 h-3 rounded-full transition-all duration-300 ease-in-out ${
